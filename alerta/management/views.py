...
def housekeeping():
    ...
    if expired_threshold_hrs:
        expired_threshold = expired_threshold_hrs * 60 * 60  # convert seconds to hours
    else:
        expired_threshold = current_app.config['DELETE_EXPIRED_AFTER']  # seconds

    if info_threshold_hrs:
        info_threshold = info_threshold_hrs * 60 * 60
    else:
        info_threshold = current_app.config['DELETE_INFO_AFTER']
